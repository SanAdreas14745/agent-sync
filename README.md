...
Файловый путь поддерживается только для локальной разработки и отдельного
набора материалов:
...
Файловый registry остаётся доступным для локальной разработки и тестов.
Режим `bundled` удалён в `2.0.0`; проекты с таким config должны перейти на
Git registry.
