...
В текущей модели пакет включает canonical файловый registry: обновление версии
доставляет одновременно CLI и набор rules/skills. В будущем bundled registry
можно заменить удалённым provider-ом, не меняя формат generated-файлов в
проектах-потребителях.
...
`"type": "bundled"` читает registry, вложенный в установленный npm-пакет;
исходный репозиторий AgentSync рядом с проектом-потребителем не нужен.
Файловый путь по-прежнему поддерживается для локального пилота или отдельного
набора материалов:
...
На текущем этапе canonical `registry/` входит в архив
`@harness-system/pb-agent-sync` и читается через provider `bundled`.
Это временная схема доставки: изменение rule или skill требует выпуска новой
версии npm-пакета. Когда появится удалённый provider, проект-потребитель сможет
перейти на новый тип `registry`, а текущий файловый и bundled варианты останутся
