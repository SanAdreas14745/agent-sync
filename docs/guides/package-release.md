...
Перед выпуском обновите версию в `package.json` и `package-lock.json`.
Команда глобальной установки в README использует npm dist-tag `latest` и не
требует изменения для каждого релиза.
