...
Файловый registry остаётся доступен для локальной разработки. В репозитории CLI
для тестов используется минимальный fixture `tests/fixtures/registry/`; он не
является источником актуальных материалов и не должен дублировать canonical registry.
