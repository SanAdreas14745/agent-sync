...
## Результаты пилота после MVP

Пилот версии 2.0.1 подтвердил корректность project/stack-фильтрации и доставки
rules. До следующего релиза нужно учесть следующие изменения.

- Критично: generated Markdown должен записываться в UTF-8 без BOM. В частности,
  `SKILL.md` обязан начинаться с `---`, иначе агент не распознаёт YAML frontmatter
  и не может автоматически подобрать skill. Нужен регрессионный тест, который
  читает `SKILL.md` после записи на диск.
- References, присутствующие в skill index, должны иметь доступный агенту путь:
  либо копироваться в generated output проекта, либо индекс должен содержать путь
  к immutable cache. Выбрать и реализовать единый контракт для всех адаптеров.
- Для адаптеров, которые автоматически загружают всю папку rules (в частности,
  Claude Code), `loadMode` нельзя считать гарантией task-level lazy loading.
  Требуется либо отдельный механизм разбиения/подключения rules, либо уточнение
  документации и ограничение роста always-loaded контекста.
