      ...
      issues: [
        {
          ...
          message: `Registry root was not found: ${registryRoot}`,
        },
      ],
    };
  }
      ...
      issues: [
        {
          ...
          message: `Registry root is not a directory: ${registryRoot}`,
        },
      ],
    };
  }

  for (const filePath of findMaterialFiles(registryRoot)) {
    const sourceFile = normalizeRelativePath(relative(registryRoot, filePath));

    try {
      const skillResources = isDirectorySkillEntrypoint(sourceFile)
        ? findSkillResources(registryRoot, dirname(sourceFile))
        : undefined;
