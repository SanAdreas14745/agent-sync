...
async function runCommand(parsed: ParsedArgs): Promise<number> {
  switch (parsed.command) {
    ...
    case 'hooks':
      return hooksCommand(parsed.options, parsed.positional);
    default:
...
function hooksCommand(options: CliOptions, positional: string[]): number {
  if (positional.length !== 1 || positional[0] !== 'install') {
    console.error('Usage: ai-skills hooks install [options]');
    return 1;
  }

  return installHooksCommand(options);
}

function installHooksCommand(options: CliOptions): number {
  const projectRoot = resolve(options.projectRoot || process.cwd());
  const configPath = join(projectRoot, '.ai-skills.json');
  const configResult = readProjectConfig(configPath);
  const hookContent = createPostCheckoutHookContent();
  ...
  let hookPath: string;
  try {
    hookPath = resolvePostCheckoutHookPath(projectRoot);
  } catch (error) {
    console.error(`ERROR git_hook_path_resolution_failed: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
  ...
  if (existsSync(hookPath)) {
    ...
    if (existingContent === hookContent) {
      console.log(`AgentSync post-checkout hook is already installed: ${hookPath}`);
      return 0;
    }

    console.error(`ERROR git_hook_conflict: Refusing to overwrite existing hook: ${hookPath}`);
    console.error('Add the AgentSync hook manually, or remove the existing hook before installing.');
    return 1;
  }
  ...
  try {
    ...
    console.error(`ERROR git_hook_write_failed: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }

  console.log(`Installed AgentSync post-checkout hook: ${hookPath}`);
  return 0;
}

function resolvePostCheckoutHookPath(projectRoot: string): string {
  const configuredHooksPath = tryReadGitConfig(projectRoot, 'core.hooksPath');

  if (configuredHooksPath) {
    const hooksDirectory = isAbsolute(configuredHooksPath)
      ? configuredHooksPath
      : resolve(projectRoot, configuredHooksPath);

    if (isHuskyHooksDirectory(hooksDirectory)) {
      return join(dirname(hooksDirectory), 'post-checkout');
    }
  ...
  const gitPath = runGit(projectRoot, ['rev-parse', '--git-path', 'hooks/post-checkout']);
  return isAbsolute(gitPath) ? gitPath : resolve(projectRoot, gitPath);
}

function isHuskyHooksDirectory(hooksDirectory: string): boolean {
  const normalizedPath = hooksDirectory.replace(/\\/g, '/');
  return (
    /(?:^|\/)\.husky\/_$/.test(normalizedPath) &&
    existsSync(join(hooksDirectory, 'h'))
  );
}

function tryReadGitConfig(projectRoot: string, key: string): string | undefined {
  try {
    const value = runGit(projectRoot, ['config', '--get', key]);
...
function createPostCheckoutHookContent(): string {
  return `#!/bin/sh
# Generated by AgentSync. Runs after a Git branch checkout.
...
if [ ! -f "$project_root/.ai-skills.json" ]; then
  exit 0
fi

if ! ai-skills update --project-root "$project_root"; then
  echo "AgentSync: update failed; run ai-skills update --project-root . manually." >&2
fi
...
Commands:
  ...
  hooks install        Install Git post-checkout automation
